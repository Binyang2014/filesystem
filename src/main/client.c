...
int clent_create_file(char *file_path, char *file_name) {
	...
	MPI_Send((void*) &message, sizeof(message), MPI_CHAR, master.rank,
			CLIENT_INSTRCTION_MESSAGE, MPI_COMM_WORLD);
	do {
		MPI_Recv(tmp, 5096, MPI_CHAR, master.rank,
				CLIENT_INSTRUCTION_ANS_MESSAGE, MPI_COMM_WORLD, &status);
